...
ccflags-y += -DRTW_GIT_HASH=\"$(RTW_GIT_HASH)\"
...
# Disabled: this loop rewrites every -I flag as $(srctree)/<path>, which is
# wrong for an external module - there the include paths are relative to the
# module directory, not to the kernel tree. With $(src) = "." it resolves
# "-I./include" to "$(srctree)/include", a directory that exists and
# silently shadows the driver's own headers, breaking the build with
# "drv_types.h: No such file or directory". Kbuild already resolves
# -I$(src)/... in ccflags-y correctly, so no conversion is needed.
ifeq (1,0)
_EXTRA_CFLAGS :=
_INC_CFLAGS :=
